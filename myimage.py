...
st.set_page_config(page_title="Radha Krishna Image Processor", layout="wide")

# Title
st.title("Radha Krishna Image - Multi-Color Channel Visualizer")

# Load image from URL
@st.cache_data
def load_image():
    #url = "https://upload.wikimedia.org/wikipedia/commons/3/37/African_Bush_Elephant.jpg"
    path = r"C:\Users\WELCOME\Desktop\Python\Numpy_project\radhakrishna.jpg"
    #response = requests.get(url)
    return Image.open(path).convert("RGB")

# Load and display image
elephant = load_image()
st.image(elephant, caption="Original Radha Krishna Image", use_container_width=True)
