...
# Load dataset
df = pd.read_csv(r"C:\Users\WELCOME\Documents\Data Science\September 2025\3rd Sept-Logistic reg classification\2.LOGISTIC REGRESSION CODE\logit classification.csv")
...
# as we increase randome state to 50,100 , accuracy decreased . so randome stae=0 mean we get optimize accuracy
...
sc = Normalizer()
x_train = sc.fit_transform(x_train)
x_test = sc.transform(x_test)'''  # very less accuracy

# penality: is a regularization tecghnique. it reduce overfitting. mean it reduce paramater high coefficient to low coeff. to adjust overfitting.
...
#classifier = LogisticRegression(penalty='l2', solver='liblinear')  # less accuracy 91.5
#classifier.fit(x_train, y_train)
...
# ---------------FUTURE PREDICTION-------------------------------------------------------------------------------
df1 = pd.read_csv(r"C:\Users\WELCOME\Documents\Data Science\September 2025\3rd Sept-Logistic reg classification\2.LOGISTIC REGRESSION CODE\final1.csv")
d2 = df1.copy()
df1 = df1.iloc[:,[3,4]].values
...
d2.to_csv('final1.csv') # it store final prediction in csv format  in directory os.getcwd()


# In Regression model , we predict future using mx+c 
